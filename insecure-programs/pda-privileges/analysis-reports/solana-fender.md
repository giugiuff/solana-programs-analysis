...
**Description**: Struct 'InsecureWithdraw' has multiple Account fields without constraints to prevent duplicate accounts
...
**Description**: The instruction 'insecure_withdraw' does not validate the caller's authority. Consider adding an explicit check like 'if !ctx.accounts.authority.is_signer { return Err(...) }'.

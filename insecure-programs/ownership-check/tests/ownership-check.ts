...
// 1) INSECURE V1 — accetta ATA non posseduto dal signer
it("insecure_log_balance_v1: consente owner mismatch", async () => {
  await program.methods
    .insecureLogBalanceV1()
    .accounts({
      mint: mintPubkey,
      tokenAccount: creatorAta,            // ATA del creator
      tokenAccountOwner: hacker.publicKey, // finto owner
    })
    .signers([hacker])
    .rpc();
});

// 2) INSECURE V2 — accetta AccountInfo arbitrario (unpack) senza check
it("insecure_log_balance_v2: consente owner mismatch", async () => {
  await program.methods

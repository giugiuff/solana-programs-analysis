...
pub struct InsecureAtomicTradeTransaction {
    pub instruction: InsecureAtomicTradeInstruction,
}
...
impl TransactionHooks for InsecureAtomicTradeTransaction {
    ...
    fn transaction_invariant_check(&self) -> Result<(), FuzzingError> {
            ...
            == self.instruction.accounts.vault_b.pubkey()
        {
            return Err(FuzzingError::with_message(
                "insecure_atomic_trade accepted duplicated vault accounts",
            ));
        }

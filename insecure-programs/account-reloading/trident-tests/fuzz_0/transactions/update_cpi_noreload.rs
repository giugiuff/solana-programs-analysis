...
pub struct UpdateCpiNoreloadTransaction {
    pub instruction: UpdateCpiNoreloadInstruction,
}
...
impl TransactionHooks for UpdateCpiNoreloadTransaction {
    ...
    fn transaction_invariant_check(&self) -> Result<(), FuzzingError> {
        ...
        if before_account.input != after_account.input {
            return Err(FuzzingError::with_message(&format!(
                "Metadata not reloaded after CPI: cached input {}, on-chain {} (new_input {})",
                before_account.input, after_account.input, self.instruction.data.new_input
            )));
        }
    ...
    fn transaction_error_handler(&self, error: TransactionError) {
        eprintln!("update_cpi_noreload failed: {error:?}");
    }
}
